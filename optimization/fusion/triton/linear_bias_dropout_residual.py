...
@jit
def _kernel(A, B, C, R, Bias, M, N, K,  #
            stride_am, stride_ak,  #
            stride_bk, stride_bn,  #
            stride_cm, stride_cn,  #
    ...
    R = R + (ram[:, None] * stride_cm + rbn[None, :] * stride_cn)
    Bias = Bias + rbn[None, :]
    # acc = tl.zeros((BLOCK_M, BLOCK_N), dtype=dot_out_dtype)
    acc = (tl.load(R) + tl.load(Bias)).to(dot_out_dtype)
...
class _linear_bias_dropout_residual(torch.autograd.Function):
    kernel = _kernel

    _locks = {}

    @staticmethod
    def _call(a, b, r, bias, dot_out_dtype, allow_tf32, fp8_fast_accum):
        original_a = a
        a = a if a.ndim == 2 else a.flatten(0, -2)
        r = r if r.ndim == 2 else r.flatten(0, -2)
        device = a.device
        ...
        N, _ = b.shape
        # allocates output
        if a.dtype in [tl.float8e4b15, tl.float8e5] or\
           b.dtype in [tl.float8e4b15, tl.float8e5]:
        ...
        if dot_out_dtype is None:
            ...
            if dot_out_dtype == torch.float16:
                ...
                dot_out_dtype = tl.int32
        ab_dtype = True
        if a.dtype in [tl.float8e5] and b.dtype in [tl.float8e5]:
            ab_dtype = False
        ...
        c = c if original_a.ndim == 2 else c.view(*original_a.shape[:-1], N)
        return c

    @staticmethod
    def forward(ctx, a, b, r, bias, dot_out_dtype=None, allow_tf32=True, fp8_fast_accum=True):
        return _linear_bias_dropout_residual._call(a, b, r, bias, dot_out_dtype=dot_out_dtype, allow_tf32=allow_tf32, fp8_fast_accum=fp8_fast_accum)


def linear_bias_dropout_residual(
    x,
    weight,
    residual,
    bias,
    dot_out_dtype=None, allow_tf32=False, fp8_fast_accum=False
):
    return _linear_bias_dropout_residual.apply(
        x,
        weight,
        residual,
        bias,
